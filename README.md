...
## About
The Cymraeg Challenge is an interactive game where users click on cards to reveal either Welsh or English words. Users then have to try and match the word to its translation. 
It is a combination of a memory and a language game, as users will have a short amount of time to view the words before they are hidden from view. When users match the cards correctly, the cards will turn green, and points will be added to the score at the bottom of the page. When a user has incorrectly chosen a word pair, the card will ‘flip back over’, hiding the words on them and the game continues.  On finding all the word pairs, a notification congratulating the user on completing the game and asking if they want to play again will appear. 
For new users, there will be a ‘how to play’ link in the header which shows instructions of how to play when clicked on. The purpose of this game is not only to be entertaining but also to boost user's welsh language skills. Welsh is one of the oldest languages in the UK and thanks to intensive education, has experienced a resurgence. Every school child in Wales must learn Welsh at least to a GCSE level. There is a rich culture of Welsh language poetry, music and plays, so learning Welsh can be beneficial on both a personal and professional level for many people. Educational tools such as the Cymraeg Challenge can make learning a new language fun and accessible.
...
* A timer to measure progress
* Ability to revise what pairs at the end of the game
...
how to text is available to view at any time, including during the game. When hovered over, the how to play button is red and the cursor changes to a pointer, to indicate that it is clickable. The logo is bright red and contrasts well against the background, while the how to play button swaps between off white and red depending on hover status.

#### Responsiveness
The game is designed to work on 320px screens upwards. This has been achieved using media queries. A max width has been set in the root directory. At the max-width, the main body of the game, the nav bar and footer will stop expanding, remaining in the center of the screen. The game has been tested on screens up to 5000px in width using DevTools. 
...
#### Interactive Cards
Fresh cards are generated everytime a game is begun. They begin with an attractive daffodil background and grow when hovered over. On a click, their inner words are revealed on a dark red background. When two cards are clicked that DO NOT match, the words remain visible for a short period of time before disappearing. If a correct guess is made, the backgrounds change to green and the words remain visible. 
The game ends when the user matches all 12 cards.

#### Congratulations Text
At the end of the game, users are shown the congratulation text, which also shows how long it took them to complete the challenge and a table of translations for the words they have just covered. At the bottom of this screen, a play again button prompts users to continue playing. 
...
### OS Compatibiility
The website has been tested on iOS and android using Lambdatest. Apart from similar issues with the iOS to safari, the game is playable on both.
